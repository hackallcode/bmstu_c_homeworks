...
int initData(data * data) {
	*data = (int *)malloc(sizeof(int) * 3);
	if (*data == NULL) return e_NoMemory();
	(*data)[0] = 0;
	(*data)[1] = 0;
	(*data)[2] = 0;
	return SUCCESS;
...
int setData(data * destData, data sourceData) {
	...
	*destData = (int *)malloc(sizeof(int) * 3);
	if (*destData == NULL) return e_NoMemory();
...
void fwriteData(data data, FILE * file) {
	fwrite(&data[0], sizeof(int), 1, file);
	fwrite(&data[1], sizeof(int), 1, file);
	fwrite(&data[2], sizeof(int), 1, file);
}

int freadData(data * data, FILE * file) {
	int return_code = SUCCESS;
	return_code = fread(&(*data)[0], sizeof(int), 1, file);
	if (return_code < 0) return return_code;
	return_code = fread(&(*data)[1], sizeof(int), 1, file);
	if (return_code < 0) return return_code;
	return_code = fread(&(*data)[2], sizeof(int), 1, file);
	if (return_code < 0) return return_code;
